...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "labx.conf"));
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the LABX and zLABX money supply statistics") + " " + _("on startup"));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, labx, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero, precompute, staking)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all LABX specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-pivstake=<n>", strprintf(_("Enable or disable staking functionality for LABX inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zpivstake=<n>", strprintf(_("Enable or disable staking functionality for zLABX inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzpiv=<n>", strprintf(_("Enable automatic wallet backups triggered after each zLABX minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-precompute=<n>", strprintf(_("Enable precomputation of zLABX spends and stakes (0-1, default %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-zpivbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zLABX backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizelabxamount=<n>     " + strprintf(_("Keep N LABX anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
            FormatParagraph(strprintf(_("Copyright (C) %i The Stakinglab Core Developers"), COPYRIGHT_YEAR)) + "\n" +
            "\n" +
...
/** Sanity checks
 *  Ensure that LABX is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Stakinglab Core is shutting down."));
...
#endif
    // Make sure only a single LABX process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Stakinglab Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //LABX: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //LABX specific: zerocoin and spork DB's
                ...
                // LABX: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false) || reindexZerocoin) {
                    ...
                    RecalculateLABXSupply(reindexZerocoin ? Params().Zerocoin_StartHeight() : 1);
                }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Stakinglab Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Stakinglab Core to complete") << "\n";
        ...
        zwalletMain = new CzLABXWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zLABXWallet
        uiInterface.InitMessage(_("Syncing zLABX wallet..."));
...
//    }
//
//    nAnonymizeStakinglabAmount = GetArg("-anonymizelabxamount", 0);
//    if (nAnonymizeStakinglabAmount > 999999) nAnonymizeStakinglabAmount = 999999;
//    if (nAnonymizeStakinglabAmount < 2) nAnonymizeStakinglabAmount = 2;
    ...
    LogPrintf("Anonymize LABX Amount %d\n", nAnonymizeStakinglabAmount);
    ...
    /* Denominations
       ...
       For example:
       1LABX+1000 == (.1LABX+100)*10
       10LABX+10000 == (1LABX+1000)*10
    */
    ...
    */
    ...
    if (pwalletMain) {
        ...
        if (GetBoolArg("-precompute", true)) {
            // Run a thread to precompute any zLABX spends
