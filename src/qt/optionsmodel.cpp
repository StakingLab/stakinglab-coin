...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeStakinglabAmount"))
        settings.setValue("nAnonymizeStakinglabAmount", 1000);

    nAnonymizeStakinglabAmount = settings.value("nAnonymizeStakinglabAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeStakinglabAmount"))
        SoftSetArg("-anonymizelabxamount", settings.value("nAnonymizeStakinglabAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in labx.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeStakinglabAmount:
            return QVariant(nAnonymizeStakinglabAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeStakinglabAmount:
            nAnonymizeStakinglabAmount = value.toInt();
            settings.setValue("nAnonymizeStakinglabAmount", nAnonymizeStakinglabAmount);
            emit anonymizeStakinglabAmountChanged(nAnonymizeStakinglabAmount);
            break;
        case CoinControlFeatures:
