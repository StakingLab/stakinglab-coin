...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a LABX address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no LABX: URI
...
{
    // Convert labx:// to labx:
    //
    //    Cannot handle this later, because labx:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open labx.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "LABX.lnk";
}
...
{
    return GetAutostartDir() / "labx.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a labx.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=LABX\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the labx app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add labx app to startup item list
