...
CzLABXWallet::CzLABXWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZLABXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZLABXSeed_deprecated()) {
                    LogPrintf("%s: Updated zLABX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzLABXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZLABXCount(nCountLastUsed);
    else if (!walletdb.ReadZLABXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzLABXWallet::Lock()
{
    seedMaster = 0;
}

void CzLABXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzLABXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZLABX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzLABXWallet::LoadMintPoolFromDB()
{
...
void CzLABXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzLABXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzLABXWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzLABXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZLABX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzLABXWallet::SeedToZLABX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzLABXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzLABXWallet::GenerateDeterministicZLABX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzLABXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzLABXWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzLABXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
