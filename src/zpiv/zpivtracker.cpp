...
CzLABXTracker::CzLABXTracker(std::string strWalletFile)
{
...
CzLABXTracker::~CzLABXTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzLABXTracker::Init()
{
...
bool CzLABXTracker::Archive(CMintMeta& meta)
{
...
bool CzLABXTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzLABXTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzLABXTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzLABXTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzLABXTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzLABXTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzLABXTracker::GetSerialHashes()
{
...
CAmount CzLABXTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzLABXTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzLABXTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzLABXTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzLABXTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzLABXTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzLABXTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzLABXTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzLABXTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzLABXTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzLABXTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzLABXWallet* zLABXWallet)
{
    bool iszLABXWalletInitialized = (NULL != zLABXWallet);
    CMintMeta meta;
    ...
    if (! iszLABXWalletInitialized)
        zLABXWallet = new CzLABXWallet(strWalletFile);
    meta.isSeedCorrect = zLABXWallet->CheckSeed(dMint);
    if (! iszLABXWalletInitialized)
        delete zLABXWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzLABXTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLABXTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzLABXTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzLABXTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzLABXTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzLABXTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzLABXWallet* zLABXWallet = new CzLABXWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zLABXWallet);
        }
        delete zLABXWallet;
