...
bool IsStandardTx(const CTransaction& tx, string& reason)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        if (txin.scriptSig.IsZerocoinSpend() || txin.IsZerocoinPublicSpend() )
            continue;
...
//                LogPrintf("%s \n",  bitcoinAddress2.ToString());
                ...
                if (pindexPrev->nHeight < Params().MinimumStakingAmountV2BlockStart() && inputSize < Params().MinimumStakingAmount()) {
                    return state.Invalid(error("CheckInputs(): tried to stake with smaller than minimum amount"));
                } else if(pindexPrev->nHeight >= Params().MinimumStakingAmountV2BlockStart() && inputSize < Params().MinimumStakingAmount2()) {
                    return state.Invalid(error("CheckInputs(): tried to stake with smaller than minimum amount"));
