...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 LABX at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zLABX send
enum ZerocoinSpendStatus {
    ZLABX_SPEND_OKAY = 0,                            // No error
    ZLABX_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZLABX_WALLET_LOCKED = 2,                         // Wallet was locked
    ZLABX_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZLABX_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZLABX_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZLABX_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZLABX_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZLABX_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZLABX_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZLABX_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZLABX_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZLABX_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZLABX_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZLABX_SPENT_USED_ZLABX = 14,                      // Coin has already been spend
    ZLABX_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZLABX_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZLABXOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzLABXTracker> zpivTracker;
    ...
    void setZWallet(CzLABXWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpivTracker = std::unique_ptr<CzLABXTracker>(new CzLABXTracker(strWalletFile));
    }

    CzLABXWallet* getZWallet() { return zwalletMain; }
    ...
    /** zLABX reset */
    boost::signals2::signal<void()> NotifyzLABXReset;
