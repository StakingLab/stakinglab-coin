...
public:
    ...
    bool WriteZLABXSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZLABXSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZLABXSeed_deprecated(uint256& seed);
    bool EraseZLABXSeed();
    bool EraseZLABXSeed_deprecated();

    bool WriteZLABXCount(const uint32_t& nCount);
    bool ReadZLABXCount(uint32_t& nCount);
